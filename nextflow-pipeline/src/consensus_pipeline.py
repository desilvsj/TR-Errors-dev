"""
Double Consensus Builder Pipeline (Step 1)

Overview:
The purpose of this segment of the pipeline is to identify the repeating sequence 
in the rolling circle amplification (RCA) sequencing read. The algorithm approaches
this by selecting a range of the first "k" nucleotides and looking for when this 
"k-mer" occurs next. The user is allowed to specify both the value of k (defaults 
to 20) and the error margin (maximum number of mismatches allowed) within that k-mer 
window (defaults to 2).

Once the length of the repeat unit has been found it then proceeds to build a 
"consensus" based on the quality scores of the repeats. It does so by using the (R1)
read to build a quality index matrix 
### EXPLAIN WHAT THIS MATRIX IS !!!
for every position n of the repeat unit 
(i.e., consensus). Once the matrix has been assembled, it then takes the highest 
scored base for every position n to build the consensus. If there are two scores of
the exact same quality score for that position, it is randomly assigned. 

Given that this program is built for paired-end sequencing, the consensus is only
completed once the program integrates the information from R2. Once the R1 matrix is
built, the program builds a temporary consensus and proceeds to align it with the 
corresponding R2 read. It is important to note that the R2 read may be the reverse-
complement (RC) and so the program verifies this by initially finding the orientation
of the first 10000 reads and makes the assumption that the rest follow suit. Once 
the consensus has been aligned, the phase shift/phi (i.e., the nucleotide offset phi 
such that the consensus base [0] aligns with the R2 at base [phi]) is calculated. 
This is then used to update the consensus matrix with the quality index scores of R2.

Once the matrix has all the information from both R1 & R2, the consensus can be built.
Note that the corresponding scoring is found by taking the sum of the scores of the bases
that match that position (Refer links in README.md regarding Illumina quality scoring).
However, given the nature of the next phase of the pipeline, this program returns what
we termed the "double consensus" (DC); which, as the name suggests, is simple the
consensus duplicated back to back. For instance, if the consensus was ACGTGCA, then
the double consensus would be ACGTGCAACGTGCA.

The program finally returns the following:
- a gzipped fastq file with the read's ID, double consensus, and corresponding quality score
- a metadata file that contains the read's ID, consensus length, and R2's phase shift (phi)


Purpose:
    Implements a ***repeat-phasing and consensus-building system for paired-end
    RCA sequencing data. The pipeline:
    1. Reads paired FASTQ files.
    2. Detects repeat unit length in reads.
    3. Builds a base-quality-weighted consensus.
    4. Aligns the second read (R2) to match phase with R1.
    5. Outputs a double-length consensus sequence and metadata.

Use:
    Called by main.py with input FASTQ files and parameters. Produces a gzipped
    FASTQ with double consensus sequences and a metadata file containing read ID,
    consensus length, and phase shift.
"""
# ----------------------- Library Imports -------------------------
...
# Lookup table mapping ASCII characters for nucleotides to integer codes
# A=0, C=1, G=2, T=3, N=4 (N used for unknown bases)
BASE2IDX = np.full(256, 4, dtype=np.int8)  # Default value 4 for 'N' and invalid bases
BASE2IDX[ord('A')] = 0
BASE2IDX[ord('C')] = 1
BASE2IDX[ord('G')] = 2
BASE2IDX[ord('T')] = 3
...
# Reverse mapping from integer codes to base characters
...
class FastqStream:
    """
    Iterates over two paired-end FASTQ files (R1 and R2) in sync.
    Handles both plain-text and gzipped FASTQ.

    Methods:
        __iter__() -> Yields tuples of (SeqRecord R1, SeqRecord R2) sequentially.
        sample(n) -> Returns first n read pairs without consuming iterator.
    """
...
class ConsensusMatrix:
    """
    Builds a position-specific quality matrix for one repeat unit.

    Attributes:
        d: Length of repeat unit.
        quality_matrix: 4×d matrix, summing quality scores for A/C/G/T at each position.
        coverage_vector: Number of reads contributing to each position.

    Methods:
        update(seq, qual, shift=0): Adds one read's contribution to the matrix.
        to_consensus(): Returns consensus sequence, quality scores, full matrices.
        consensus_length(): Returns repeat length d.
    """
...
class RepeatDetector:
    """
    Detects the repeat length d in a read.
    Uses a rolling k-mer match starting from the first k bases.
    Allows up to max_errors mismatches.
    Raises NoRepeats if no repeat is found.

    --> Why k-mer? Faster than full alignment for detecting repeat boundaries.
    --> max_errors: Allows handling sequencing noise.
    """
...
class OrientationDecider:
    """
    Determines whether R2 reads should be reverse complemented globally.
    Compares alignment quality (phred-weighted) in both orientations over a sample.
    Returns 'RC' if reverse complement scores higher.

    - Reason: Avoids per-read orientation guessing; uses sample to decide for all.
    - Method:
        1. Build consensus from R1.
        2. Score R2 vs consensus and R2 reverse complement vs consensus.
        3. Compare counts.
    """
...
class PhaseAligner:
    """
    Finds the phase shift (phi) where R2 best aligns with the consensus.
    Two-step approach:
        1. Fast scan using lazy_best_shift (Numba-accelerated) within a window.
        2. Fallback to mismatch-minimizing, quality-aware search.
    """
...
class ResultWriter:
    """
    Writes output double-consensus sequences (FASTQ) and metadata.
    Metadata format: read_id, consensus length, phase shift.
...
class RepeatPhasingPipeline:
    """
    Orchestrates the full repeat phasing process:
        - Reads paired FASTQ.
        - Determines repeat length d.
        - Builds R1 consensus.
        - Aligns R2 and merges into consensus.
        - Outputs double consensus sequence + metadata.
    Parameters control k-mer size, max errors, sample size, and output paths.
    """
